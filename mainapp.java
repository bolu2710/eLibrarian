...
public class mainapp {
	...
	public static void main(String[] args) throws Exception {
		...
		while (run) { // Start up loop
			// run welcome display
			speech.main(args);
			Translator.translate("en", "es", "Welcome to eLibrarian. Press L to login, C to create account or A to login");
			...
			if (w.getInput().equals(l)) {
				...
				if (log0.HasRegistry()) {
					...
					speech_login.main(args);
					Translator.translate("en", "es", "Welcome to the main menu, Press B to view books, Press S to search, Press U to view account info");
					System.out.println("________________________________________________________________________\n" +
							"| |------------------------------------------------------------------| |\n" +
							"| |                           Login success.                         | |\n" +
					...
					while (mmrun) { // MAIN MENU LOOP asks user for B, S or U
						LoggedIn li = new LoggedIn(a2);// get user's choice to look at books, search for specific book,
														// or view account info
						...
						if (li.getChoice().equals(listbooks)) {
							...
							while (dbrun) { // DATABASE LOOP gets books from database
								...
								System.out.println(
										"________________________________________________________________________\n" +
												...
												+
												"|_|__________________________________________________________________|_|\n");
								status = 1;// resets the user's status on if they've rented a book or joined its
											// waitlist
								while (brun) { // BOOK LOOP asks user for book id
									...
									if (vb.getInput().equals(q)) { // if they press Q to quit
										quit_speech.main(args);
										Translator.translate("en", "es", "Goodbye. Hope to see you soon");
										...
										System.out.println(
												"________________________________________________________________________\n"
														...
														"| | Current Waitlist: " + userBook.getWL()
														+ " user(s)                   \n" +
														"| |                                                                  | |\n"
														+
														"| | 'C' to cancel.                                      'Q' to quit. | |\n"
										...
										while (arun) {// ACTION LOOP asks user for W or B
											Action action = new Action(userBook.getAvailability());// start action
																									// prompt

											// IF book UNAVAILABLE AND they enter W and FIRST TRY
											if (!userBook.getAvailability() && action.getInput().equals(wait)
													&& status == 1) {
												wtime = userBook.getWL() * 21;// wait time = 3 weeks times amount of
																				// people on list
												userBook.changeWL(userBook.getWL() + 1, vb.getInput());// update
																										// waitlist to
																										// include user
																										// BEFORE
																										// finding wtime
												System.out.println(
														"________________________________________________________________________\n"
																...
																"| |------------------------------------------------------------------| |\n"
																+
																"| |       Thank you for joining the waitlist. You are number "
																+ userBook.getWL() + ".      | |\n" +
																"| |               The estimated wait time is " + wtime
																+ " day(s).              | |\n" +
																"|_|__________________________________________________________________|_|\n");
											...
											else if (!userBook.getAvailability() && action.getInput().equals(wait)
													&& status == 2) {
												System.out.println(
														"________________________________________________________________________\n"
																+
																"| |------------------------------------------------------------------| |\n"
																+
																"| |       You are already on the waitlist. You are number "
																+ userBook.getWL() + ".      \n" +
																"| |              The estimated wait time is " + wtime
																+ " day(s).              \n" +
																"|_|__________________________________________________________________|_|\n");
											...
											else if (userBook.getAvailability()
													&& action.getInput().equals(listbooks)) {
												System.out.println(
														"________________________________________________________________________\n"
																+
																"| |------------------------------------------------------------------| |\n"
																...
																+
																"|_|__________________________________________________________________|_|\n");
												userBook.changeAvailability(false, vb.getInput());// close book's
																									// availablity
												userBook.changeWL(userBook.getWL() + 1, vb.getInput());// update
																										// waitlist
											...
											// If book UNAVAILABLE OR AVAILABLE and they enter C, then cancel or go back
											// to database
											else if ((!userBook.getAvailability() || userBook.getAvailability())
													&& action.getInput().equals(c)) {
												wait(2000);// wait 2 seconds to print database
							...
							while (srun) {
								...
								// If info matches book title OR author in system
									...
									searchinfo = searchlist.searchInventory(bs.getSInput());// get string form of list
																							// from booklist.java
									System.out.println(
											"________________________________________________________________________\n"
													+
									...
									while (sbrun) { // Search BOOK LOOP asks user for book id
										ViewBook vb2 = new ViewBook();// ask user to select a book by id FROM SEARCH
																		// RESULTS
										if (vb2.getInput().equals(q)) { // if they press Q to quit
											...
											System.out.println(
													"________________________________________________________________________\n"
															...
															"| | Current Waitlist: " + userBook2.getWL()
															+ " user(s)                   \n" +
															"| |                                                                  | |\n"
															+
															"| | 'C' to cancel.                                      'Q' to quit. | |\n"
											...
											while (arun2) {// ACTION LOOP asks user for W or B
												Action action2 = new Action(userBook2.getAvailability());// start action
																											// prompt

												// IF book UNAVAILABLE AND they HAVENT borrowed it ANYWHERE, and they
												// enter W
												if (!userBook2.getAvailability() && action2.getInput().equals(wait)
														&& searchwaitliststatus == 1) {
													wtime2 = userBook2.getWL() * 21;// wait time = 3 weeks times amount
																					// of people on list
													userBook2.changeWL(userBook2.getWL() + 1, vb2.getInput());// update
																												// waitlist
																												// to
																												// include
																												// user
												...
												// IF book UNAVAILABLE AND they are ALREADY ON waitlist from joining in
												// search or database
												else if (!userBook2.getAvailability() && action2.getInput().equals(wait)
														&& (searchwaitliststatus == 2 || status == 2)) {
													System.out.println(
															"________________________________________________________________________\n"
																	+
																	"| |------------------------------------------------------------------| |\n"
													...
													// sbrun = false;//exit book view loop
													// GO BACK TO SEARCH RESULTS
												} // IF book UNAVAILABLE AND they are ALREADY BORROWED BOOK in search OR
													// database
												else if (!userBook2.getAvailability() && action2.getInput().equals(wait)
														&& (searchwaitliststatus == 3)) {
													System.out.println(
															"________________________________________________________________________\n"
																	+
																	"| |------------------------------------------------------------------| |\n"
												...
												else if (userBook2.getAvailability()
														&& action2.getInput().equals(listbooks)) {
													System.out.println(
															"________________________________________________________________________\n"
																	+
																	"| |------------------------------------------------------------------| |\n"
																	...
																	+
																	"|_|__________________________________________________________________|_|\n");
													userBook2.changeAvailability(false, vb2.getInput());// close book's
																										// availablity
													userBook2.changeWL(userBook2.getWL() + 1, vb2.getInput());// update
																												// waitlist
													searchwaitliststatus = 3;// creates new waitlist output if you
																				// borrowed first and tried to join wl
																				// instead of join wl first and try
																				// again
												...
												// If book UNAVAILABLE OR AVAILABLE and they enter C, then cancel or go
												// back to search results
												else if ((!userBook2.getAvailability() || userBook2.getAvailability())
														&& action2.getInput().equals(c)) {
													wait(2000);// wait 2 seconds to print database
												...
												// IF viewing Search results (entering id) and press C, then go back to
												// SEARCH
						...
						} else if (li.getChoice().equals(u)) { // if they entered the wrong password but right email,
																// prompt them for password again.
							urun = true;
							uirun = true;
							while (urun) {// USER INFO loop
								String userinfo = "";
								User user = new User();// run user to list database of users
								userinfo = user.info(log0.getEmail());// get string form of user's info from the user
																		// table
								System.out.println(
										"________________________________________________________________________\n" +
												"| |------------------------------------------------------------------| |\n"
				...
				} else if (log0.HasEmail()) { // if they entered the wrong password but right email, prompt them for
												// password again.
					a = 2;
					uname = log0.getEmail(); // send correct email to next login
				...
				while (cwrun) {
					speech_create.main(args);
					Translator.translate("en", "es", "Welcome to create account. Press L key to go back and login. Press C to continue");
					System.out.println("________________________________________________________________________\n" +
							"| |------------------------------------------------------------------| |\n" +
					...
					if (createwelcome.getInput().equals(l)) {
						...
						// run create
						while (crun) {
							Create ca = new Create(createattempts, wtime2);
				...
				while (librun) {// librarian admin screen loop
					speech_admin.main(args);
					Translator.translate("en", "es", "Welcome to admin screen. Press L to go back, Press C to continue as a librarian");
					larun = true;
					...
					if (libwelcome.getInput().equals(l)) {
						...
						while (larun) {// librarian action loop
							...
							if (la.HasRegistry() && la.in.equals(add)) { // if admin exists, login as librarian AND ADD
																			// then add book
								...
								// send newest id to iterate database. Should fill values accordingly with NEW
								// BOOK
								System.out.println(
										"________________________________________________________________________\n" +
							...
							} else if (la.HasRegistry() && la.in.equals(edit) && la.in2.equals("Delete")) { // if admin
																											// exists,
																											// login as
																											// librarian
																											// AND
								...
								adminBook.binfo(la.getId());// get info for book matching the entered ID
								// send newest id to iterate database. Should fill values accordingly with BOOK
								// to delete
								System.out.println(
										"________________________________________________________________________\n" +
							...
							} else if (la.HasEmail()) { // if they entered the wrong password but right email, prompt
														// them for password again.
								a = 2;
								uname = la.getEmail(); // send correct email to next login
							} else if (!la.HasEmail()) { // if they entered an invalid email, prompt them for input
															// again
								a = 3;
								System.out.println("\nThat account does not exist. Please Retry.");
							} else {
				...
				quit_speech.main(args);
				Translator.translate("en", "es", "Goodbye, you will be missed");
				System.out.println("Quitting...Goodbye!");
